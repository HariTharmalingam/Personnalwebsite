...
$siteOwnersEmail = 'contact@harishanth-tharmalingam.com';
...
if($_POST) {
   ...
   // Check Name
    if (strlen($name) < 2) {
        $error['name'] = "Veuillez écrire votre nom.";
    }
    // Check Email
    if (!preg_match('/^[a-z0-9&\'\.\-_\+]+@[a-z0-9\-]+\.([a-z0-9\-]+\.)*+[a-z]{2}/is', $email)) {
        $error['email'] = "Veuillez écrire une adresse mail valide.";
    }
    // Check Message
    if (strlen($contact_message) < 15) {
        $error['message'] = "Veuillez écrire votre message";
    }
   // Subject
    if ($subject == '') { $subject = "Envoyer"; }
